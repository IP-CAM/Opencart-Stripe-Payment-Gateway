<?php
require_once(DIR_SYSTEM.'/library/stripe/lib/init.php');
class ControllerExtensionPaymentTmdStripePayment extends Controller {
    ...
    public function index() {
        ...
        $data['entry_card_digits'] = $this->language->get('entry_card_digits');
        $data['entry_card_expire_date'] = $this->language->get('entry_card_expire_date');
        $data['entry_card_my'] = $this->language->get('entry_card_my');
        $data['text_error'] = $this->language->get('text_error');
        $data['error_expire_date'] = $this->language->get('error_expire_date');
        $data['error_card_cvc'] = $this->language->get('error_card_cvc');
    ...
    public function send() {
        ...
        if(!empty($this->request->get['stripeToken'])) {
            ...
            if(!empty($output['status']=='success')) {
                $json['status'] = 'success';
                ...
                $comment = 'orderHash : '.$orderHash.'</br>'.'clientSecret : '.$clientSecret.'</br>'.'Creat Date : '.date('Y-m-d');
                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_tmd_stripe_payment_order_status_id'), $comment, false , false);
